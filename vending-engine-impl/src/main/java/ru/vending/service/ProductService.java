...
public class ProductService {
    ...
    public void buyProductById (Long id, String paymentType) {
        Product product = productRepository.findById(id).orElseThrow(EntityNotFoundException::new);
        if (product.getCount() == 0) {
            //todo exception
            log.info("Sorry! This product is out of stock");
        } else {
            Integer prodPrice = product.getPrice();
            ...
            if (payResponse.equals("s")) {
                comportInterfaceIntegration.spiralMotorInput(product.getChoiceNumber());
                Integer spiralMotorResponse = comportInterfaceIntegration.spiralMotorWaiting();
